...
X, y = make_classification(n_samples=100,
    n_features=50000, n_informative=50, n_redundant=0, n_classes=2,
    random_state=42)
...
if key_vals_output is not None:
    ...
    ###########################################################################
    # Optional: check distributed mapper == Centralized
    # Centralized Mapper: input key/value pairs => output key/value pairs
    start_time = time.time()
    key_vals_output_single = MapReduce(n_jobs=5, pass_key=True, verbose=0).map(fit_predict, key_values_input)
    print("# Centralized mapper completed in %.2f sec" % (time.time() - start_time))
    cv_scores_single = reduce_cv_classif(key_vals_output_single, cv_dict, y_true=y)

    cv_scores.sort_values(["param_0", "fold"], inplace=True, ignore_index=True)
    cv_scores_single.sort_values(["param_0", "fold"], inplace=True, ignore_index=True)
    assert np.all(cv_scores == cv_scores_single), "Centralized mapper != distributed mapper"
